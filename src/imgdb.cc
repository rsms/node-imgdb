...
int addImageBlob(const int dbId, const long int id, const void *blob, size_t length) {
  Image image(Magick::Blob(blob, length));
...
int loaddbfromstream(const int dbId, std::ifstream& f, srzMetaDataStruct& md) {
  ...
  if (md.iskVersion < SRZ_V0_6_0) {
    ...
    // read sigs
    for (sigMap::size_type k = 0; k < szt; k++) {
...
int loadalldbs(char* filename) {
  ...
  for (int k = 0; k < sz; k++) { // for each db
    ...
    count += loaddbfromstream(dbId, f, md);
  }
...
std::vector<double> queryImgID(const int dbId, long int id, unsigned int numres) {
...
std::vector<double> queryImgIDFast(const int dbId, long int id, unsigned int numres) {
  /*query for images similar to the one that has this id
  numres is the maximum number of results
   */
  if (!validate_imgid(dbId, id)) { cerr << "ERROR: image id (" << id << ") not found on given dbid (" << dbId << ") or dbid not existant" << endl ; return std::vector<double>();};
...
// query by keywords
std::vector<int> mostPopularKeywords(const int dbId, std::vector<long int> imgs, std::vector<int> excludedKwds, int count, int /*mode (unused)*/) {
...
// query by keywords
std::vector<double> queryImgIDKeywords(const int dbId, long int id, unsigned int numres, int kwJoinType, int_vector keywords){
  ...
  if (id == 0) { // random images with these kwds
    ...
    for (unsigned int var = 0; var < min(V.size()/2, numres); ) { // var goes from 0 to numres
      unsigned int rint = rand()%(V.size()/2);
...
//std::vector<double> queryImgIDFastKeywords(const int dbId, long int id, unsigned int numres, int kwJoinType, int_vector keywords){
std::vector<double> queryImgIDFastKeywords(const int dbId, long int id, unsigned int /*numres*/, int /*kwJoinType*/, int_vector /*keywords*/){
  if (!validate_imgid(dbId, id)) { cerr << "ERROR: image id (" << id << ") not found on given dbid (" << dbId << ") or dbid not existant" << endl ; return std::vector<double>();};
  throw string("not yet implemented");
}
...
std::vector<double> queryImgDataFastKeywords(const int dbId, int *, int *, int *, double *, unsigned int, int, int, std::vector<int>){
  if (!validate_dbid(dbId)) { cerr << "ERROR: database space not found (" << dbId << ")" << endl; return std::vector<double>();}
  throw string("not yet implemented");
}
